 ...
 * Сопоставление статусов API → разделы кабинета:
 *   Предзаказ              → preOrder=true или будущая plannedDeliveryDate
 *   Упаковка               → ACCEPTED_BY_MERCHANT (приняли, но ещё не собрали)
 *   Передача               → ASSEMBLED (собрали, но не передали)
 *   Переданы на доставку   → PACKAGE_REGISTERED, HANDED_OVER_TO_COURIER, DELIVERED
 *   Отменены при доставке  → CANCELLING (отмена в процессе)
 *   Новые                  → APPROVED_BY_BANK (ждут принятия)
 */
function kaspiSection(o, now) {
  ...
  if (st === 'ACCEPTED_BY_MERCHANT') return 'packing';
  if (st === 'ASSEMBLED') return 'transfer';
