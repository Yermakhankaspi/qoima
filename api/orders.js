...
function bucketFor(order, now) {
  ...
  if (st === 'ACCEPTED_BY_MERCHANT') {
    // Уже отдан курьеру → В доставке (явная дата передачи)
    const transmissionDate = a.kaspiDelivery?.courierTransmissionDate;
    if (transmissionDate) {
      return 'delivery';
    }
    // Собран, ждёт курьера → Передача
    // Поле может прийти как boolean, число или строка
    const isAssembled = a.assembled === true || a.assembled === 'true' || a.assembled === 1;
    if (isAssembled) return 'transfer';
    // Иначе → Упаковка
    return 'packing';
  }
...
export default async function handler(req, res) {
  ...
  try {
    // Заказы должны показывать ТЕКУЩЕЕ СОСТОЯНИЕ, как в Kaspi кабинете —
    // не зависят от периода. Берём 90 дней назад (захватывает все активные).
    // Параметр days оставлен для совместимости, но используется только для completed.
    const days = parseInt(req.query.days || '30', 10);
    const clamped = Math.min(Math.max(days, 1), 90);
    const now = Date.now();
    const completedFrom = now - clamped * MS_PER_DAY;  // период для "Выданы"
    const activeFrom = now - 90 * MS_PER_DAY;          // для активных всегда 90 дней

    const allOrders = await getOrders(activeFrom, now);
